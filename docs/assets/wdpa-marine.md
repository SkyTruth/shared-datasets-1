...
notes: Monthly job preserves source fields and publishes FGB plus PMTiles. The 2026-06-05 reviewed metadata-contract release
  ...
  tiles with feature_id and ext_id only. The release preserves 331 upstream invalid geometries from the current FGB; no geometry
  repair was applied. Release history, source versions, row counts, and file hashes are recorded in the bucket release index
  and per-run records.
row_count: 17657
data_profile:
  field_count: 33
