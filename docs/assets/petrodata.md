...
data_profile:
  ...
  - field: source_layer + PRIMKEY
    ...
    duplicate_row_count: 6
    status: non_unique
    notes: Composite still duplicates the three preserved source key pairs, so it is not used as a provider feature ID.
  - field: feature_id
    distinct_values: 1273
    duplicate_value_count: 0
    duplicate_row_count: 0
    status: unique
    notes: Generated per-feature ID approved for the 2026-06-05 metadata-contract release after provider IDs were ruled out.
feature_metadata:
...
- path: latest/petrodata.pmtiles
  format: pmtiles
  role: companion
  purpose: Metadata-lookup tiles generated from the same combined polygons with only feature_id and ext_id properties
...
| `latest/petrodata.pmtiles` | `pmtiles` | `companion` | Metadata-lookup tiles generated from the same combined polygons with only feature_id and ext_id properties |
