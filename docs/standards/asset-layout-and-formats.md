...
- `geometry_hash`: content fingerprint over canonical geometry and the stable
  geometry-equivalence key. Consumers may use it after loading the metadata
...
| `{asset-slug}.metadata.ndjson.gz` | Canonical durable metadata sidecar, one JSON object per `feature_id`, with top-level `feature_id`, `geometry_hash`, and `properties_hash`. |
