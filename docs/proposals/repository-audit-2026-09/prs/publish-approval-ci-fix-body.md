...
## CI correction (1d276ba)

The introducing PR exposed two CI failures before tests could run: Gitleaks misclassified the deterministic synthetic `proposal_key` fixture digest, and the alert workflow attempted to run a newly added validator from its trusted base checkout. Running the remaining CI stages locally also exposed a static guardrail that recognized only literal main checkouts, not the pinned main-workflow executor.

The follow-up commit:

- Extends default Gitleaks rules with one exception requiring both the exact fixture path and exact public synthetic digest. Other values and other paths remain scanned. The same failing Git history was retested with CI's pinned Gitleaks 8.24.3, including positive detection controls.
- Routes possible mutation plans through a separate read-only job, without checking out PR code. Ordinary code PRs skip privileged preview; JSON plans, rename/delete evidence, and all supported body-fence forms still reach trusted-base validation. API errors, incomplete files, and changed PR identity refuse routing.
- Validates main and the exact workflow source before immutable bootstrap checkout. Static guardrails recognize only that bounded contract, rejecting arbitrary refs, ignored failures, misplaced guards, shell overrides, or alternative checkout repositories/paths. Existing authorization and captured-executor verification remain intact.

Final local validation: **919 tests and 669 subtests passed**, four unavailable-native-tool skips and the documented unchanged `/private/tmp`-sensitive Terraform fixture deselected; Ruff, static/diff guardrails, admission check, committed whitespace check, and pinned secret scan passed. Behavioral tests execute the routing JavaScript and the actual shell guards. Live GitHub checks are being verified against this pushed commit.

The original rollout prerequisites remain. No merge, deployment, settings mutation, or canonical GCS write is part of this correction.
