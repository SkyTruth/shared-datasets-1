...
class CatalogWebPmtilesJavascriptTests(unittest.TestCase):
    ...
    def test_catalog_viewer_can_zoom_to_selected_map_assets(self):
        ...
        self.assertIn("fetch(url, { cache: \"no-store\" })", map_preview)
        self.assertIn("flatgeobuf.deserialize(response.body)", map_preview)
        self.assertIn("/api/download-url?", map_preview)
        self.assertIn("credentials: \"include\"", map_preview)
        ...
        self.assertNotIn("asset.public_url || \"\", asset.canonical_path", map_preview)
