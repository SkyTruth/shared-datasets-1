...
class PublishingConciergeTests(unittest.TestCase):
    ...
    def test_field_profile_reports_decision_table_statistics(self):
        ...
        translation_fields = [candidate.field for candidate in options.translation_field_candidates]
        self.assertEqual(translation_fields[0], "NAME")
        self.assertNotIn("source_id", translation_fields)
        self.assertNotIn("DISC", translation_fields)
    ...
    def test_petrodata_like_recommendations_keep_table_compact(self):
        ...
        translation_fields = [candidate.field for candidate in options.translation_field_candidates]
        self.assertIn("NAME", translation_fields)
        self.assertIn("COUNTRY", translation_fields)
        self.assertIn("RESINFO", translation_fields)
        self.assertNotIn("PRIMKEY", translation_fields)
        self.assertNotIn("LAT", translation_fields)
        self.assertNotIn("LONG", translation_fields)
        self.assertNotIn("SOURCEINFO", translation_fields)
    ...
    def test_build_artifacts_commands_include_machine_translation_helper_for_autogenerated_fields(self):
        commands = publishing_concierge.commands_for_build_artifacts(self._autogenerated_localized_state())
        rendered = "\n".join(commands)

        self.assertIn("scripts/feature_metadata_machine_translate.py", rendered)
        self.assertIn("--with deep-translator", rendered)
        self.assertIn("--locale es", rendered)
        self.assertIn("--locale fr", rendered)
        self.assertIn("--field name", rendered)
        self.assertIn("scripts/feature_metadata_localization.py", rendered)
