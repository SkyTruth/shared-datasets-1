...
class FeaturePreviewTests(unittest.TestCase):
    ...
    def test_preview_terraform_uses_isolated_preview_resources(self):
        ...
        self.assertNotIn('resource "google_storage_bucket" "preview_bucket"', main_tf)
        self.assertNotIn('resource "google_firestore_database" "feature_preview"', main_tf)
        self.assertIn("from = google_storage_bucket.preview_bucket", main_tf)
        ...
        self.assertIn("local.preview_loader_service_account", outputs_tf)
    ...
    def test_preview_deploy_workflow_keeps_control_plane_separate_from_source_ref(self):
        ...
        self.assertNotIn("Release stable preview bootstrap state", workflow)
        self.assertNotIn('terraform -chdir="${PREVIEW_TERRAFORM_DIR}" state rm', workflow)
        self.assertIn("Reset stable preview data", workflow)
        self.assertIn("scripts/feature_preview_reset.py", workflow)
        self.assertIn("if: ${{ env.PREVIEW_DATA_MODE == 'reset' }}", workflow)
        self.assertNotIn("Terraform reset plan", workflow)
        self.assertNotIn("Wait for preview database ID reuse", workflow)
    ...
    def test_preview_destroy_workflow_only_runs_destroy_path(self):
        ...
        self.assertNotIn("Release stable preview bootstrap state", workflow)
        self.assertNotIn("terraform -chdir=terraform/envs/preview state rm", workflow)
        self.assertIn("Reset stable preview data", workflow)
        self.assertIn("scripts/feature_preview_reset.py", workflow)
        ...
        self.assertNotIn("google_storage_bucket.preview_bucket", destroy_allowlist_section)
        self.assertNotIn("google_firestore_database.feature_preview", destroy_allowlist_section)
        self.assertIn("google_cloud_run_v2_service.feature_preview_service", workflow)
        self.assertIn("google_cloud_run_v2_service.feature_preview_catalog_viewer", workflow)
        ...
        self.assertIn("INDEX_LOADER_SERVICE_ACCOUNT: feature-preview-loader@shared-datasets-1.iam.gserviceaccount.com", workflow)
    ...
    def test_preview_terraform_iam_sync_uses_custom_role_and_allowlist(self):
        ...
        self.assertIn("serviceAccounts/${module.feature_preview_service_account.email}", preview_terraform_tf)
        self.assertIn("serviceAccounts/${module.feature_preview_loader_service_account.email}", preview_terraform_tf)
        ...
        self.assertIn('resource "google_storage_bucket" "feature_preview"', preview_terraform_tf)
        self.assertIn('resource "google_firestore_database" "feature_preview"', preview_terraform_tf)
        self.assertIn('delete_protection_state     = "DELETE_PROTECTION_ENABLED"', preview_terraform_tf)
        self.assertIn('deletion_policy             = "ABANDON"', preview_terraform_tf)
        self.assertNotIn("storage.buckets.create", preview_terraform_tf)
        self.assertNotIn("datastore.databases.create", preview_terraform_tf)
        self.assertNotIn("iam.serviceAccounts.create", preview_terraform_tf)
        self.assertIn("run.services.create", preview_terraform_tf)
        self.assertIn("iap.webServices.setIamPolicy", preview_terraform_tf)
        self.assertNotIn("resourcemanager.projects.setIamPolicy", preview_terraform_tf)
        self.assertIn("preview_iap_services_only", preview_terraform_tf)
        self.assertIn("roles/iam.serviceAccountUser", preview_terraform_tf)
