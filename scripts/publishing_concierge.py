...
def render_pr_body_from_state(
    state: dict[str, Any], *, publish_plan: dict[str, Any] | None = None
) -> str:
    plan = plan_from_state(state)
    metadata = canonical_resolve_metadata(step_record(state, "resolve-metadata").get("evidence", {}))
    ...
    rendered_plan = reviewed_dataset_plan.render_document(document)
    body = f"""## Summary
...
def validate_state_for_pr(state: dict[str, Any]) -> dict[str, Any]:
    ...
    if not errors and is_canonical_publish_workflow(state):
        try:
            publish_plan = build_publish_plan_from_state(state)
            if "plan_document" in state:
                from scripts import reviewed_dataset_plan

                recorded = state["plan_document"]
                document = reviewed_dataset_plan.normalize_document(
                    {
                        "plan_version": 1,
                        "finalization_version": reviewed_dataset_plan.FINALIZATION_VERSION,
                ...
                if Path(recorded["path"]).read_bytes() != expected or recorded[
                    "sha256"
                ] != reviewed_dataset_plan.sha256(expected):
                    raise WorkflowError(
                        "checked-in plan/evidence changed; start a fresh concierge proposal before render-pr, or use reviewed_dataset_plan.py prepare for a new reviewed PR"
                    )
    ...
    document = reviewed_dataset_plan.normalize_document(
        {
            "plan_version": 1,
            "finalization_version": reviewed_dataset_plan.FINALIZATION_VERSION,
            "publish": result["publish_plan"],
        }
    )
    path = reviewed_dataset_plan.write_document(document, repo_root=args.repo_root)
    state["plan_document"] = {
        "path": str(path.resolve()),
        "sha256": reviewed_dataset_plan.sha256(path.read_bytes()),
    }
    save_state(args.state_file, state)
...
def build_workflow_parser() -> argparse.ArgumentParser:
    ...
    render_pr.add_argument(
        ...
        type=Path,
        default=REPO_ROOT,
        help="Repository receiving the checked-in immutable plan document.",
    )
    ...
    retry = subparsers.add_parser(
        "refresh-retry-plan",
        help="Prepare an UNAPPROVED replacement plan; changed expectations require a new reviewed PR.",
    )
