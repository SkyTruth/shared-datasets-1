...
- `.claude/skills/update-feature-metadata-translations/SKILL.md` before
  ...
  fields, if any, should be autogenerated, and record explicit deferral when no
  preview translations should be generated.
...
- For first preview loads of release-oriented vector metadata, either
  ...
  CSV, or an explicit maintainer decision to defer translations for this
  preview bundle.
...
with `pmtiles convert`. For the final `.pmtiles`, confirm the PMTiles magic
bytes, run `pmtiles verify`, inspect `pmtiles show` metadata, and decode
representative tiles to verify expected layers and compact feature properties.
...
- For first preview loads of release-oriented feature metadata, stop for the
  ...
  that the maintainer chose to defer translations for the preview.
