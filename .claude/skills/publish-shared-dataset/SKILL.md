...
- PMTiles display artifacts must preserve compact feature properties for the
  catalog inspector. Use the repo vector helper's GDAL MBTiles to PMTiles
  conversion path; direct Tippecanoe PMTiles output is disabled until a future
  Tippecanoe version is proven to generate valid PMTiles directly.
...
```
  ...
  `review_state`, and `notes`; then run
  ...
  generated sidecars are complete, unless the maintainer explicitly defers
  translation work.
...
6. For first uploads of release-oriented vector assets, ask which locales and
   ...
   `{asset-slug}.metadata-translations.csv` with those rows and run the
   localization generator before staging publish candidates.
