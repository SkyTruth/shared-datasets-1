...
### Geospatial CLI toolchain

Use `uv run python ...` as the required entry point for repo-owned Python
helpers, including upload, validation, catalog, raster, and vector build
scripts. `uv` owns the Python environment and should be the first place to
centralize Python package behavior.

Native geospatial command-line tools such as `gdalinfo`, `ogrinfo`, `ogr2ogr`,
`tippecanoe`, and `pmtiles` are external binaries unless the repo explicitly
declares and verifies them as part of a pinned toolchain. Running
`uv run ogr2ogr ...` does not by itself prove that GDAL is uv-managed; it may
still resolve a Homebrew, conda, system, or user-installed binary from `PATH`.
Treat a native CLI as centrally managed only when the binary path and version
come from a repo-declared dependency, repo-documented tool directory, or pinned
container image.

Do not let native geospatial tool versions be implicit when generating
publishable data. Before a manual build that depends on these tools, run or
record the versions and paths resolved under the repo environment:
...
Repo helpers that shell out to geospatial CLIs should report the resolved tool
versions and paths in their dry-run/build output. README update notes, catalog
notes, run records, or PR descriptions should include the tool versions for
manual publishes when the generated bytes depend on those tools.

For reproducibility-sensitive uploads, scheduled ingestion jobs, or any build
where different local GDAL/Tippecanoe/PMTiles versions could plausibly change
output bytes, use a repo-owned pinned toolchain instead of an ad hoc local
`PATH`. Preferred options, in order, are:

1. A production ingestion Dockerfile for scheduled jobs.
2. A repo-documented local container/toolchain command that pins GDAL,
   Tippecanoe, and PMTiles versions.
3. A repo-declared `uv` dependency or tool only when it actually provides the
   required native binaries reproducibly on supported platforms, not merely
   Python bindings to an ambient system GDAL.

Local `PATH` binaries are acceptable for exploratory inspection and low-risk
manual publishes only when their paths and versions are recorded. If multiple
users are expected to regenerate identical bytes, promote the workflow to a
pinned container or other repo-declared toolchain before publishing.

Do not install or upgrade global geospatial tooling silently as part of a
dataset publish. If a missing binary blocks a manual task, ask or use an
approved explicit install command, then record the installed version.
...
| Build local vector publish artifacts | `scripts/vector_asset.py` | Keeps generated data out of git and standardizes FGB + PMTiles creation |
...
4. Prepare generated publish artifacts outside the repo tree. For vector assets, use `uv run python scripts/vector_asset.py build ...`; it writes final upload candidates under `$TMPDIR/shared-datasets-1/vector-assets/{asset-slug}/publish/` by default, with intermediates under `build/`. Override the temp root with `SHARED_DATASETS_WORKDIR` or pass `--work-dir` when needed.
